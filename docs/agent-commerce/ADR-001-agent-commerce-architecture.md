...
| 5 | Expose the service via both the existing HTTP endpoint and a Cloudflare MCP `paidTool` on the same Worker, sharing facilitator/price config | **Blocked** — see [MCP-PAIDTOOL-BLOCKER.md](./MCP-PAIDTOOL-BLOCKER.md) | Attempted 14 Aug 2026: `agents@0.20.1` requires `zod@^4.0.0` and `react@^19.0.0` as hard peer deps; this repo's `x402-hono` tree is locked to `zod@3.25.76`. `npm install` refused via ERESOLVE — not forced. Needs either an `agents` release that relaxes the zod peer range, or an `x402-hono` upgrade that moves off zod v3, before this can be built without risking cross-version schema bugs. |
